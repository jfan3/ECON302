...
HP_LAMBDA = 400*PERIOD; % The lambda parameter for the HP-Filter
N_LEADS_LAGS = 6;  % calculate autocorr. table up to N_LEADS_LAGS-1 leads and lags.
TOL =  0; % .0001;  % If PP has roots below TOL in absolute value,
...
svv_fil = [];
im = sqrt(-1);
PP_may_be_singular = ( min(abs(eig(PP))) < TOL );
