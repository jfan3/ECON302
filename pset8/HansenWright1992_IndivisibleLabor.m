...
cy_bar  = 0.78-delta*ky_bar; % steady state consumption to output ratio
...
for t = 2:2000
    zz(t) = rhoz*zz(1,t-1) + randn*sigma_eps;
...
title('Indivisible Labor');
xlabel('Hours');
ylabel('Prod.');
axis([-10 10 -10 10]);

%% Discussion
% While the marginal utility of leisure is diminishing in the standard
% model, in the indivisible labor model the marginal utility of leisure is
% simply a constant. In response to a positive productivity shock, the
% substitution effect (which dominates here) says that the representative
% consumer will substute away from leisure (wage goes up, so opportunity
% cost of leisure also goes up) and toward consumption. As a result, the
% representative consumer will increase hours worked. In the baseline model
% the marginal utility of leisure rises as the consumer works more, which 
% dampens this response. In the indivisible labor model, the marginal
% utility of leisure remains constant as the consumer increases her time
% spent in market labor. As a result, hours worked are more volatile in the
% indivisible labor model, moving the model closer to the data along this
% dimension.